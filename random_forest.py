...
def main(args):
    ...
    mgf_file = os.path.join(args.path, "%s/mgf_feat.npy" % (args.dataset_name.replace("-", "_")))
    maccs_file = os.path.join(args.path, "%s/maccs_feat.npy" % (args.dataset_name.replace("-", "_")))
    mgf_feat = np.load(mgf_file)
    maccs_feat = np.load(maccs_file)

    dataset = GraphPropPredDataset(name=args.dataset_name, root=args.path)
    smiles_file = os.path.join(args.path, "%s/mapping/mol.csv.gz" % (args.dataset_name.replace("-", "_")))
