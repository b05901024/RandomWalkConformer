...
def main(dataset_name, path):
    dataset = GraphPropPredDataset(name=dataset_name, root=path)

    df_smi = pd.read_csv(os.path.join(path, f"{dataset_name}/mapping/mol.csv.gz".replace("-", "_")))
    ...
    save_path = os.path.join(path, f"{dataset_name}".replace("-", "_"))
...
if __name__=="__main__":
    ...
    main(args.dataset_name, args.path)